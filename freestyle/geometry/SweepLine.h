...
public:
    ...
    */
    ...
    // This function is called for each point, in order. "segments" are all segments that touch this point.
    // Hence, if a segment ends at p, then it can be removed from the active set of segments.

    inline void process(Point& p,
                        vector<Segment<T,Point>*>& segments,
                        Vec3r viewpoint,
                        binary_rule<Segment<T,Point>,Segment<T,Point> >& binrule,
                        bool addSegs=true, bool removeSegs=true
            )
    {
                    ...
                    )
    {
            ...
            s++)
        {
            ...
            if(GeomUtils::intersect2dSeg2dSegParametric(v0, v1, v2, v3, t, u) == GeomUtils::DO_INTERSECT)
                // note: the old code assumed 2D t and u coordinates, now new code returns 3D coords
                // however, the old code was treating the 2D coords as if they were 3D anyway
                //      if ((*S).edge()->intersectParametric(*((*currentS).edge()), viewpoint, t, u))
            {
                //	const double eps = 0.001;
                //	assert(t >= -eps && t <= 1+ eps && u >= -eps && u < 1+ eps);
                // fails if both segments are degenerate
    ...
    inline void remove(Segment<T,Point>* s)
    {
        if(s->intersections().size() > 0)
            _IntersectedEdges.insert(s);
        _set.remove(s);
    }

    set<Segment<T,Point>* >& intersectedEdges() {return _IntersectedEdges;}
...
private:
    ...
    std::set<Segment<T,Point>* > _IntersectedEdges; // the list of intersected edges
