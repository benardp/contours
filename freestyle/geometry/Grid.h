...
public:
  ...
  inline bool inGrid(const Vec3r & p)
  {
    int tmp;
    for (int i = 0; i < 3; i++) {
  ...
  */
  ...
  inline Vec3u getNumCells() const {
    return _cells_nb;
  }
  ...
  public:
    ...
    virtual bool operator !=(const iterator &) const { printf("undefined 1\n"); };
    virtual Vec3u operator *() const { printf("undefined 2\n"); return Vec3u(-1,-1,-1); };
    virtual Vec3u dereference() const { printf("undefined 2\n"); return Vec3u(-1,-1,-1); };
  };
  ...
  */
  ...
  /*
  // iterator to visit all cells intersecting a given triangle
  class tri_iterator
  {
  public:
    tri_iterator(Vec3r p1, Vec3r p2, Vec3r p3);
    void operator++();
    bool done();
    Cell * operator*() const;
  private:
  ...
  */
  ...
  void triangleIntersections(Vec3r p1,Vec3r p2,Vec3r p3, set<Polygon3r*> & possibleIntersections);
