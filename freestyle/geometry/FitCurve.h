...
typedef struct Point2Struct {	/* 2d point */
  ...
  inline real x() const {return coordinates[0];}
  inline real y() const {return coordinates[1];}
	} Point2;
...
public:
   ...
   */
  void FitCurve(std::vector<Vec2d>& data, std::vector<Vec2d>& oCurve, real error);
protected:
    /* Vec2d	*d;			  Array of digitized points	  */
    /* int		nPts;		  Number of digitized points	*/
    /* real	error;		User-defined error squared	*/
  void FitCurve(Vector2 *d, int nPts, real error);
  ...
  /* real	error;		  User-defined error squared	   */
  void FitCubic(Vector2 *d, int first, int last, Vector2 tHat1, Vector2 tHat2, real error);
