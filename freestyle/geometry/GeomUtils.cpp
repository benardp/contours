...
#define AXISTEST_Z0(a, b, fa, fb)                          \
  ...
  bool intersectRayTriangle(Vec3r& orig, Vec3r& dir,
    ...
    // Added by Aaron.  Why wasn't this here before???
    if ( t < 0.0)
      return false;
  ...
  // Intersection between plane and ray in point-normal form, in hopes of fixing a particular numerical issue
  intersection_test intersectRayPlanePN(Vec3r& orig, Vec3r& dir,
				      Vec3r& norm, Vec3r & ptOnPlane,
				      real& t,
				      real epsilon) { 
    Vec3r dp= orig - ptOnPlane;
    real denom = norm * dir;
    real ndotd = norm * dp;
  ...
  // Intersection between plane and ray in point-normal form, in hopes of fixing a particular numerical issue
  intersection_test intersectLinePlanePN(Vec3r& orig, Vec3r& dir,
					 Vec3r& norm, Vec3r & ptOnPlane,
					 real& t,
					 real epsilon) { 
    Vec3r dp= orig - ptOnPlane;
...
bool closestPoints3dLines(const Vec3r p1, const Vec3r dir1, const Vec3r p2, const Vec3r dir2,
			  Vec3r & pt1, Vec3r & pt2)
{
  // http://books.google.com/books?id=WGpL6Sk9qNAC&lpg=PA146&ots=Pl5OgM3jkO&dq=closest%20points%20on%20two%20lines&pg=PA147#v=onepage&q=closest%20points%20on%20two%20lines&f=false
  // Crister Ericson, Real-time Collision Detection, p.147
  ...
  if (d < .001)  // lines are parallel, so there is an infinite family of solutions
    return false;
...
intersection_test intersect3dLines(const Vec3r p1, const Vec3r p2, const Vec3r q1, const Vec3r q2, 
				   Vec3r & pt)
{
  Vec3r dirp = p2 - p1;
  Vec3r dirq = q2 - q1;
  ...
  Vec3r pint, qint;
  if (!closestPoints3dLines(p1, dirp, q1, dirq, pint, qint))
    {
      // doesn't catch the case where they overlap
      return DONT_INTERSECT;
    }

  double dist = (qint - pint).norm();
...
intersection_test intersect3dLineSegments(const Vec3r p1, const Vec3r p2, const Vec3r q1, const Vec3r q2, 
					  Vec3r & pt)
{
  Vec3r dirp = p2 - p1;
  Vec3r dirq = q2 - q1;
  ...
  if (!closestPoints3dLines(p1, dirp, q1, dirq, pint, qint))
    {
      // not catching case where they are parallel
	return DONT_INTERSECT;
    }

  // --- clip to the line segments ----

  double dp = (pint - p1) * dirp;
  ...
  // --- check if the clipped points are the same ---
