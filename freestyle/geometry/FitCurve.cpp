...
static real *Reparameterize(Vector2 *d, int first, int last, real *u, BezierCurve bezCurve);
static real NewtonRaphsonRootFind(BezierCurve Q, Vector2 P, real u);
static Vector2 BezierII(int degree, Vector2 *V, real t);
static real B0(real u);
static real B1(real u);
static real B2(real u);
static real B3(real u);
static Vector2 ComputeLeftTangent(Vector2 *d, int end);
static Vector2 ComputeLeftTangent(Vector2 *d, int end);
static Vector2 ComputeLeftTangent(Vector2 *d, int end);
static real ComputeMaxError(Vector2 *d, int first, int last, BezierCurve bezCurve, real *u, int *splitPoint);
static real *ChordLengthParameterize(Vector2 *d, int first, int last);
static BezierCurve  GenerateBezier(Vector2 *d, int first, int last, real *uPrime, Vector2 tHat1, Vector2 tHat2);
static Vector2 V2AddII(Vector2 a, Vector2 b);
static Vector2 V2ScaleIII(Vector2 v, real s);
...
real V2SquaredLength(Vector2 *a) 
{	return(((*a)[0] * (*a)[0])+((*a)[1] * (*a)[1]));
}
	
/* returns length of input vector */
real V2Length(Vector2 *a) 
{
	return(sqrt(V2SquaredLength(a)));
}

Vector2 *V2Scale(Vector2 *v, real newlen) 
{
  real len = V2Length(v);
...
real V2Dot(Vector2 *a, Vector2 *b) 
{
	return(((*a)[0]*(*b)[0])+((*a)[1]*(*b)[1]));
}

/* return the distance between two points */
real V2DistanceBetween2Points(Vector2 *a, Vector2 *b)
{
real dx = (*a)[0] - (*b)[0];
real dy = (*a)[1] - (*b)[1];
...
//    real	*uPrime;		/*  Parameter values for region */
//    Vector2	tHat1, tHat2;	/*  Unit tangents at endpoints	*/
{
    ...
    real 	C[2][2];			/* Matrix C		*/
    real 	X[2];			/* Matrix X			*/
    real 	det_C0_C1,		/* Determinants of matrices	*/
    	   	det_C0_X,
	   		det_X_C1;
    real 	alpha_l,		/* Alpha values, left and right	*/
    	   	alpha_r;
...
/*					C[1][0] += V2Dot(&A[i][0], &A[i][1]);*/	
    ...
    if (alpha_l < 1.0e-6 || alpha_r < 1.0e-6) {
		real	dist = V2DistanceBetween2Points(&d[last], &d[first]) /
					3.0;
...
//    Vector2	*d;			/*  Array of digitized points	*/
//    int		first, last;		/*  Indices defining region	*/
//    real	*u;			/*  Current parameter values	*/
//    BezierCurve	bezCurve;	/*  Current fitted curve	*/
{
    int 	nPts = last-first+1;	
    int 	i;
    real	*uPrime;		/*  New parameter values	*/

    uPrime = (real *)malloc(nPts * sizeof(real));
...
//    real 	t;		/* Parametric value to find point for	*/
{
    int 	i, j;		
...
{
    int		i;	
    real	*u;			/*  Parameterization		*/

    u = (real *)malloc((unsigned)(last-first+1) * sizeof(real));
...
//    real	*u;			/*  Parameterization of points	*/
//    int		*splitPoint;		/*  Point of maximum error	*/
{
    int		i;
    real	maxDist;		/*  Maximum error		*/
    real	dist;		/*  Current error		*/
...
void FitCurveWrapper::FitCurve(vector<Vec2d>& data, vector<Vec2d>& oCurve, real error)
{
  int size = data.size();
  ...
  // added by Aaron, since it looks like there's a memory leak here.
  delete [] d;
}

void FitCurveWrapper::FitCurve(Vector2 *d, int nPts, real error)
{
...
void FitCurveWrapper::FitCubic(Vector2 *d, int first, int last, Vector2 tHat1, Vector2 tHat2, real error)
{
    BezierCurve	bezCurve; /*Control points of fitted Bezier curve*/
    real	*u;		/*  Parameter values for point  */
    real	*uPrime;	/*  Improved parameter values */
    real	maxError;	/*  Maximum fitting error	 */
    int		splitPoint;	/*  Point to split point set at	 */
    int		nPts;		/*  Number of points in subset  */
    real	iterationError; /*Error below which you try iterating  */
    ...
    if (nPts == 2) {
	    real dist = V2DistanceBetween2Points(&d[last], &d[first]) / 3.0;
	...
	// added by Aaron to fix a memory leak
	free((void *)bezCurve);
