...
IndexedFaceSet::IndexedFaceSet(real *iVertices, unsigned iVSize,
                                ...
                                int * iFaceUserData, float * iVertexUserData,
                                unsigned iCopy, bool meshSilhouettes)
    : Rep()
{
    if(1 == iCopy)
    {
        ...
        _FaceUserData = new int[_NumFaces];
        memcpy(_FaceUserData, iFaceUserData, _NumFaces*sizeof(int));

        _VertexUserData = new float[_VSize/3];
        memcpy(_VertexUserData, iVertexUserData, (_VSize/3) * sizeof(float));
    ...
    else
    {
        ...
        _FaceUserData = iFaceUserData;
        _VertexUserData = iVertexUserData;
    }

    //  for(int i=0;i<10;i++)
    //    printf("IFS constructor[%d]= %d\n", i, _FaceUserData[i]);
    ...
    :Rep(iBrother)
{
    ...
    _FaceUserData = new int[_NumFaces];
    memcpy(_FaceUserData, iBrother._FaceUserData, _NumFaces*sizeof(int));

    _VertexUserData = new float[_VSize/3];
    memcpy(_VertexUserData, iBrother._VertexUserData, (_VSize/3)*sizeof(float));
...
IndexedFaceSet::~IndexedFaceSet()
{
    ...
    if (_FaceUserData != NULL)
    {
        delete [] _FaceUserData;
        _FaceUserData = NULL;
    }

    if (_VertexUserData != NULL)
    {
        delete [] _VertexUserData;
        _VertexUserData = NULL;
    }
