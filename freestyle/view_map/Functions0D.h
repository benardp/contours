...
namespace Functions0D {
  ...
  class LIB_VIEW_MAP_EXPORT IsophoteDistanceF0D : public UnaryFunction0D<real>
  {
  private:
    double _isovalue, _maxDistance;
  public:
    IsophoteDistanceF0D(double rho, double maxDist) { _isovalue = rho; _maxDistance = maxDist; }
    string getName() const { return "IsophoteDistanceF0D"; }
    real operator()(Interface0DIterator & iter);
  };
  
  class LIB_VIEW_MAP_EXPORT ImageSpaceNormalF0D : public UnaryFunction0D<Vec2f>
  {
    string getName() const { return "ImageSpaceNormalF0D"; }
    Vec2f operator()(Interface0DIterator & iter);
  };
