...
void checkSVs(vector<SVertex*>&ioSVertices)
{
  for(vector<SVertex*>::iterator svit = ioSVertices.begin(); svit != ioSVertices.end(); ++svit)
    {
      SVertex * sv = *svit;

      const vector<FEdge*> & fes = sv->fedges();

      // not invalid; just a temporary test
      if ((fes.size() == 2) && 
	  (fes[0]->getNature() & Nature::SILHOUETTE) &&
	  (fes[1]->getNature() & Nature::SILHOUETTE) &&
	  (fes[0]->getFace1() != fes[1]->getFace1()))
	assert(sv->viewvertex() == NULL);
...
ViewEdge * ViewEdgeXBuilder::BuildSmoothViewEdge(const OWXFaceLayer& iFaceLayer){
  ...
  // is it a closed loop ?
  //  if((first == end) && (size != 1)){
  if (FindNextFaceLayer(end) == first)
    {
    ...
    //    assert(fefirst->vertexA() != fe->vertexB());
...
ViewEdge * ViewEdgeXBuilder::BuildSharpViewEdge(const OWXEdge& iWEdge) {
  ...
  //  if((firstWEdge == endWEdge) && (size!=1)){
    //    assert(0);  // Aaron: doesn't seem like this could ever be reached, because of the way the userdata flag is set.
  
  if (FindNextWEdge(endWEdge) == firstWEdge)
    {
    fefirst->SetPreviousEdge(fe);
    fe->SetNextEdge(fefirst);
    newVEdge->SetA(0);
...
SVertex * ViewEdgeXBuilder::MakeSVertex(Vec3r& iPoint){
  ...
  // Aaron:  OMG, do they really do this instead of using the topology?
  // Wonder if it causes any problems...
