...
protected:
  map<unsigned int, real* > _mapping; // for each vector the list of nbOrientations weigths corresponding to its contributions to the nbOrientations directional maps
  unsigned _nbOrientations;
  ImagePyramid **_imagesPyramids; // the pyramids of images storing the different SVM

  // internal
  real _bound; // cos(Pi/N)
...
public:
   ...
   */
  real* AddFEdge(FEdge *iFEdge);

  /*! Compute the weight of direction dir for orientation iNOrientation */
  real ComputeWeight(const Vec2d& dir, unsigned iNOrientation);
