...
void SteerableViewMap::Clear(){
  ...
  if(!_mapping.empty()){
    for(map<unsigned int, real*>::iterator m=_mapping.begin(), mend=_mapping.end(); 
    m!=mend;
    ++m){
      delete [] (*m).second;
    }
...
real SteerableViewMap::ComputeWeight(const Vec2d& dir, unsigned i){ 
  real dotp = fabs(dir*_directions[i]);
  if(dotp < _bound)
    return 0;
  if(dotp>1)
    dotp = 1;
...
real * SteerableViewMap::AddFEdge(FEdge *iFEdge){
  unsigned i;
  unsigned id = iFEdge->getId().getFirst();
  map<unsigned int, real* >::iterator o = _mapping.find(id);
  if(o!=_mapping.end()){
    return (*o).second;
  }
  real * res = new real[_nbOrientations];
  for(i=0; i<_nbOrientations; ++i){
    res[i] = 0;
  }
