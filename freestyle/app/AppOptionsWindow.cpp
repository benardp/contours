...
void AppOptionsWindow::Propagate() {
  ...
  bool ok;
  double r = sphereRadiusLineEdit->text().toFloat(&ok);
  if(ok)
    g_pController->setSphereRadius(r);
  else
    sphereRadiusLineEdit->setText(QString(QString::number((double)g_pController->getSphereRadius())));
  r = krEpsilonLineEdit->text().toFloat(&ok);
  if(ok)
    g_pController->setSuggestiveContourKrDerivativeEpsilon(r);
  else
    krEpsilonLineEdit->setText(QString(QString::number((double)g_pController->getSuggestiveContourKrDerivativeEpsilon())));
}
...
void AppOptionsWindow::Cancel() {
  ...
  sphereRadiusLineEdit->setText(QString::number((double)g_pController->getSphereRadius()));
  krEpsilonLineEdit->setText(QString(QString::number((double)g_pController->getSuggestiveContourKrDerivativeEpsilon())));
