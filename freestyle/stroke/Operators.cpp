...
void Operators::create(UnaryPredicate1D& pred, vector<StrokeShader*> shaders) {
  ...
  for (Operators::I1DContainer::iterator it = _current_set->begin();
    ...
    /*
      ViewEdge * ve = dynamic_cast<ViewEdge*>(*it);
      assert(ve != NULL);
      Interface0D & start = (*(ve->verticesBegin()));

      printf("[ID: %d %d, Nature: %d, %f %f %f; %f %f] ",ve->getId().getFirst(), ve->getId().getSecond(), (int)ve->getNature(),
	     start.getX(), start.getY(), start.getZ(),
	     start.getProjectedX(), start.getProjectedY()
);
      */
