...
Strip::Strip(const vector<StrokeVertex*>& iStrokeVertices, bool hasTips, bool beginTip, bool endTip){
  vector<StrokeVertex*> newVerts;

  vector<StrokeVertex*>::const_iterator v = iStrokeVertices.begin();
  newVerts.push_back(*v);

  Vec2r pPrev((*v)->getPoint());

  // remove overlapping points

  /*
  for(v++;v!=iStrokeVertices.end();v++)
    {
      Vec2r p((*v)->getPoint());
      Vec2r dir = p - pPrev;
      
      if (dir.norm() > 0.5) //.9)//0.5)
	{
	  newVerts.push_back(*v);
	  pPrev = p;
	}
  ...
  */
  ...
  Vec2r back = iStrokeVertices.back()->getPoint();

  for(int i=0;i<iStrokeVertices.size();i++)
    {
      Vec2r p(iStrokeVertices[i]->getPoint());
      Vec2r dir = p - pPrev;
      
      if (i == 0 || i == iStrokeVertices.size() -1 ||
	  ((p - pPrev).norm() > 0.5  && (p - back).norm() > 0.5))
	{
	  newVerts.push_back(iStrokeVertices[i]);
	  pPrev = p;
	}
    }


  // hack to avoid seg fault as a result of stroke having only 1 point
  // should really find a way to get rid of these strokes perhaps
  if (newVerts.size() == 1)
    newVerts.push_back(*iStrokeVertices.begin());

  //  printf("Input # points: %d\n", iStrokeVertices.size());
  //  printf("Ouptut # points: %d\n", newVerts.size());

  createStrip(newVerts);
  if (!hasTips)
    computeTexCoord (newVerts);
  else
    computeTexCoordWithTips (newVerts, beginTip, endTip);
...
Strip::createStrip (const vector<StrokeVertex*>& iStrokeVertices)
{
  ...
  v++){
      ...
      if (dirNorm < 0.0001)
	{
	  printf("WARNING: vanishing stroke tangent\n", dirNorm);
	  //	  dir = p2 - pPrev;
	  //	  dirNorm = dir.norm();
	  continue;
	}
  ...
  /*
  // Aaron's hacky attempt to fix closed loops
  v = iStrokeVertices.begin();
  v2 = iStrokeVertices.end();
  --v2;
