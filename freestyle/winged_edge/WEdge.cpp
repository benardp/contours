...
WOEdge * WFace::MakeEdge(WVertex *v1, WVertex *v2)
{
  ...
  if(v1->GetId() == v2->GetId())
    cerr << "Warning: edge " << this << " null with vertex id " << v1->GetId() << endl;
...
WFace* WShape::MakeFace(vector<WVertex*>& iVertexList, unsigned iMaterial, int userData)
{
  // allocate the new face
  WFace *face = instanciateFace();

  return MakeFace(iVertexList, iMaterial, face, userData);
}

WFace * WShape::MakeFace(vector<WVertex*>& iVertexList, vector<Vec3r>& iNormalsList, vector<Vec2r>& iTexCoordsList, unsigned iMaterial, int userData)
{
  // allocate the new face
  WFace *face = MakeFace(iVertexList, iMaterial, userData);
...
WFace* WShape::MakeFace(vector<WVertex*>& iVertexList, unsigned iMaterial, WFace *face, int userData)
{
  ...
  vb = va;
  int i = 0;
  for(; va != iVertexList.end(); va = vb, i++)
  {
		vb++;
