...
protected:
  ...
  float _surfaceNdotV;  // N dot V from the underlying smooth or interpolated surface, as computed by the RIF

public:
  void * userdata; // designed to store specific user data
  inline WVertex(const Vec3r &v) {_Id = 0; _Vertex = v; userdata = NULL; _Shape = NULL;_Smooth=true;_Border=-1; _surfaceNdotV = -2; }
  /*! Copy constructor */
  ...
  inline void SetNormal(const Vec3r& n) {_Normal = n;}
  ...
  void SetSurfaceNdotV(float val) { _surfaceNdotV = val; }
  float GetSurfaceNdotV() const { return _surfaceNdotV; }
...
protected:
  ...
  int _RIFData; // data transferred from the RIF file to indicate the vertex-based facing
  // 1: FRONT-FACING, 2: BACK-FACING, 3: INCONSISTENT
...
public:
  ...
  void SetRIFData(int ud) { _RIFData = ud; }
  int GetRIFdata() const { if(radial()) return _RIFData-4; return _RIFData; }
  bool radial() const { return _RIFData > 3; }

  /*! The vertex of index i corresponds to the a vertex 
   *  of the edge of index i
   */
  inline WVertex* GetVertex(unsigned int index) const
  {
  ...
  Vec3r barycentricCoordinates(Vec3r point)
  {
    return GeomUtils::ComputeBarycentricCoords(GetVertex(0)->GetVertex(),
					       GetVertex(1)->GetVertex(),
					       GetVertex(2)->GetVertex(),
					       point);
  };

  Vec3r baryToPoint(Vec3r bary)
  {
    return GeomUtils::Bary2Point(GetVertex(0)->GetVertex(),
				 GetVertex(1)->GetVertex(),
				 GetVertex(2)->GetVertex(),
				 bary);
  };

  // check if a point lies inside the face (assuming the point lies on the plane of the face)
  bool pointInFace(Vec3r point)
  {
    // could probably be done more robustly using shewchuk's code 

    for(int e=0;e<3;e++)
      {
	Vec3r v1 = GetVertex(e      )->GetVertex();
	Vec3r v2 = GetVertex((e+1)%3)->GetVertex();
	Vec3r v3 = GetVertex((e+2)%3)->GetVertex();

	// do a sign test against the edge (v1,v2)

	Vec3r edgevec = v2 - v1;
	edgevec.normalize();
	Vec3r edgeNormal = v3 - v1;
	edgeNormal = edgeNormal - (edgevec * edgeNormal) * edgevec;

	bool sign1 = (v3-v1) * edgeNormal >= 0;
	bool sign2 = (point - v1)* edgeNormal >= 0;
...
public:
  ...
  virtual ~WShape()
  {
    //    printf("deleting WShape edgelist\n");
    //    fflush(stdout);
    ...
    //    printf("deleting WShape vertexlist\n");
    //    fflush(stdout);
    ...
    //    printf("deleting WShape facelist\n");
    //    fflush(stdout);
   ...
   */
  virtual WFace * MakeFace(vector<WVertex*>& iVertexList, unsigned iMaterialIndex, int userData=0);
  ...
  inline void SetMeshSilhouettes(bool b) { _meshSilhouettes = b; }
  inline bool MeshSilhouettes() const { return _meshSilhouettes; }
...
protected:
   ...
   */
  virtual WFace * MakeFace(vector<WVertex*>& iVertexList, unsigned iMaterialIndex, WFace *face, int userData =0);
};
