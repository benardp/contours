...
private:
  ...
  /*  VisT _visibility;
  bool _onContourEdge;
  vector<VisOccluder> _occluders;
  */

public:
  inline WXVertex(const Vec3r &v)
    : WVertex(v)
  {_curvatures = 0; _ndotv = -100; }
  /*! Copy constructor */
  ...
  //  inline int regionIndex() const { return _regionIndex; }
  inline void setNdotV(real ndotv) { _ndotv = ndotv; }

  inline bool isFeature();
  inline CurvatureInfo* curvatures() {return _curvatures;}
  inline real ndotv() const { return _ndotv; }
};
...
private:
  ...
  bool _regionEdge; // True if this connects vertices that are part of the same visibility region
...
public:
  ...
  {_nature = iBrother.nature();_front = iBrother._front;_order = iBrother._order;
    _regionEdge = iBrother._regionEdge; }
  ...
  inline bool isRegionEdge() const { return _regionEdge; }
  ...
  inline void SetRegionEdge(bool r) { _regionEdge = r; }  
};
...
protected:
  ...
  POBoundaryEdge * _sourcePOB; // if this face constructed from a punch-out cusp region, a pointer to the source edge.  NULL if otherwise

  //  vector<FEdgeIntersection*> _intersections; 
  // this could also be stored in _SmoothLayers, but doesn't really add much

public:
  inline WXFace() : WFace() {_Z=0.0;_front = false; _sourcePOB = NULL;}
  /*! Copy constructor */
    ...
    : WFace(iBrother)
  {
    ...
    //    _intersections.copy(iBrother._intersections.begin(), iBrother._intersections.end());
  }
  ...
  inline bool front(bool useConsistency=false) { return useConsistency && GetRIFdata() != 3 ? GetRIFdata() == 1 : _front; }
  inline bool consistent() { return front(true) == front(false); }
  ...
  POBoundaryEdge * sourcePOB() const { return _sourcePOB; }
  void SetSourcePOB(POBoundaryEdge* src) { _sourcePOB = src; }
  ...
  real interpolatedNdotV(Vec3r point)
  {
    Vec3r bary = barycentricCoordinates(point);
    real ndotv0 = ((WXVertex*)GetVertex(0))->ndotv();
    real ndotv1 = ((WXVertex*)GetVertex(1))->ndotv();
    real ndotv2 = ((WXVertex*)GetVertex(2))->ndotv();

    return bary[0] * ndotv0 + bary[1] * ndotv1 + bary[2] * ndotv2;
  };
};
