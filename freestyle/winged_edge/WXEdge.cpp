...
WXSmoothEdge* WXFaceLayer::BuildSmoothEdge(){ 
  ...
  if(ok){
	...
	// break edge for region grouping

	WEdge *eA=  _pSmoothEdge->woea()->GetOwner();
	WEdge *eB=  _pSmoothEdge->woeb()->GetOwner();
...
WFace* WXShape::MakeFace(vector<WVertex*>& iVertexList, unsigned iMaterialIndex, int userData)
{
  WFace *face = WShape::MakeFace(iVertexList, iMaterialIndex, userData);
  if(0 == face)
    return 0;
...
WFace * WXShape::MakeFace(vector<WVertex*>& iVertexList, vector<Vec3r>& iNormalsList, vector<Vec2r>& iTexCoordsList, unsigned iMaterialIndex,int userData)
{
  WFace *face = WShape::MakeFace(iVertexList, iNormalsList, iTexCoordsList, iMaterialIndex, userData);
