...
// if the RenderStyle is not NO_STYLE, then we should override any rendering options in the scene graph
//typedef enum { NO_STYLE, SHADED_STYLE, LINE_STYLE, FRONTBACK_STYLE } MeshRenderStyle;

class LIB_RENDERING_EXPORT GLRenderer : public SceneVisitor
{
 public:

  inline GLRenderer() : SceneVisitor() { _overrideColors = false; _frontBackStyle = false; }
  virtual ~GLRenderer() {}
 ...
 protected:
  ...
  inline void glMultMatrixr(double *m) {glMultMatrixd(m);}
  ...
  /*
 private:
  MeshRenderStyle _renderStyle;
 public:
  void SetRenderStyle(MeshRenderStyle style) { _renderStyle = style; }
  MeshRenderStyle GetRenderStyle() { return _renderStyle; }
  void SetupRendering();
  void PostRendering();
  */

 public:
  void SetOverrideColors(bool override) { _overrideColors = override; }
  void SetFrontBackStyle(bool fbstyle) { _frontBackStyle = fbstyle; }
  void SetFBViewpoint(Vec3r vp) { _fbViewpoint = vp; }
  
 private:
  bool _overrideColors;
  bool _frontBackStyle;
  Vec3r _fbViewpoint;
};
