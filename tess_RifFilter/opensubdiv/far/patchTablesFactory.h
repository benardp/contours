...
private:
    ...
    static real * computeFVarData(HbrFace<T> const *f, const int width, real *coord, bool isAdaptive);
    ...
    typedef PatchTypes<real *>         FVarPointers;
...
FarPatchTablesFactory<T>::Create( HbrMesh<T> const * mesh, FacesList const & flist, std::vector<int> const & remapTable, int firstLevel, FarPatchTables::Type patchType, int numPtexFaces, int fvarwidth ) {
    ...
    real         * fptr = fvarwidth>0 ? &result->_fvarData._data[0] : 0;
...
FarPatchTablesFactory<T>::computeFVarData(
    HbrFace<T> const *f, const int width, real *coord, bool isAdaptive) {
    ...
    if (isAdaptive) {
        ...
        for ( int j=0; j < nverts; ++j ) {
            ...
            real         *fvdata = v->GetFVarData(f).GetData(0);
...
FarPatchTablesFactory<T>::Splice(FarMeshVector const &meshes,
    ...
    if (fvarWidth > 0) {
        ...
        std::vector<real>::iterator FV_IT = result->_fvarData._data.begin();
        ...
        for (FarPatchTables::Descriptor::iterator it =
            ...
            for (size_t i = 0; i < meshes.size(); ++i) {
                ...
                if (parray) {
                    ...
                    std::vector<real>::const_iterator begin =
