...
struct FarPatchParam {
    ...
    struct BitField {
        ...
        /// @param v  v parameter
        ///
        void Rotate( real & u, real & v ) const;
...
FarPatchParam::BitField::GetParamFraction( ) const {
    if (NonQuadRoot()) {
        return 1.0f / real( 1 << (GetDepth()-1) );
    } else {
        return 1.0f / real( 1 << GetDepth() );
    }
}

inline void
FarPatchParam::BitField::Normalize( real & u, real & v ) const {

    real frac = GetParamFraction();

    // top left corner
    real pu = (real)GetU()*frac;
    real pv = (real)GetV()*frac;
...
inline void 
FarPatchParam::BitField::Rotate( real & u, real & v ) const {
    switch( GetRotation() ) {
         case 0 : break;
         case 1 : { real tmp=v; v=1.0f-u; u=tmp; } break;
         case 2 : { u=1.0f-u; v=1.0f-v; } break;
         case 3 : { real tmp=u; u=1.0f-v; v=tmp; } break;
         default:
             assert(0);
    }
}
