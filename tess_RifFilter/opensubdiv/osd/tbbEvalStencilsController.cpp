...
public:
    ...
    bool SetOutput(Mode mode, OsdVertexBufferDescriptor outDesc, real * outData) {
    ...
    void operator() (tbb::blocked_range<int> const &r) const {
        ...
        real * out = _outData + r.begin() * _outStride;

        for (int i=r.begin(); i<r.end(); ++i, ++sizes) {

            memset( out, 0, _length * sizeof(real) );

            for (int j=0; j<(*sizes); ++j, ++index, ++weight) {

                real const * cv = _ctrlData + (*index)*_ctrlDesc.stride;
