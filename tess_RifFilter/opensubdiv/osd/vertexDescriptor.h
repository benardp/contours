...
struct OsdVertexDescriptor {
    ...
    /// Constructor
    ///
    /// @param numVertexElem   number of vertex-interpolated data elements (reals)
    ///
    /// @param numVaryingElem  number of varying-interpolated data elements (reals)
    ///
    ...
    /// vertex.
    ///
    /// @param vertex  The real array containing the vertex-interpolated primvar
    ///                data that needs to be reset.
    ///
    /// @param varying The real array containing the varying-interpolated primvar
    ///                data that needs to be reset.
    ///
    /// @param index   Vertex index in the buffer.
    ///
    void Clear(real *vertex, real *varying, int index) const {
        if (vertex) {
            memset(vertex+index*numVertexElements, 0, sizeof(real)*numVertexElements);
        }

        if (varying) {
            memset(varying+index*numVaryingElements, 0, sizeof(real)*numVaryingElements);
    ...
    ///
    inline
    void AddWithWeight(real *vertex, int dstIndex, int srcIndex, real weight) const {
    ...
    ///
    inline
    void AddVaryingWithWeight(real *varying, int dstIndex, int srcIndex, real weight) const {
    ...
    ///
    void ApplyVertexEditAdd(real *vertex, int primVarOffset, int primVarWidth, int editIndex, const real *editValues) const {
    ...
    /// @param editValues The values to add to the primvar datum.
    ///
    void ApplyVertexEditSet(real *vertex, int primVarOffset, int primVarWidth, int editIndex, const real *editValues) const {
        int d = editIndex * numVertexElements + primVarOffset;
        for (int i = 0; i < primVarWidth; ++i) {
            vertex[d++] = editValues[i];
        }
