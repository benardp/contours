...
OsdCpuVertexBuffer::OsdCpuVertexBuffer(int numElements, int numVertices)
    ...
    _cpuBuffer = new real[numElements * numVertices];
}
...
void
OsdCpuVertexBuffer::UpdateData(const real *src, int startVertex, int numVertices) {

    memcpy(_cpuBuffer + startVertex * _numElements,
           src, GetNumElements() * numVertices * sizeof(real));
}
