    ...
    string *errorMessage)
{
    ...
    memset( _vertexBuffer->BindCpuBuffer(), 0,
            3 * fmesh->GetNumVertices() * sizeof(real));
    ...
    if (vvNames.size()) {
        ...
        memset( _vvBuffer->BindCpuBuffer(), 0,
                vvNames.size() * fmesh->GetNumVertices() * sizeof(real));
    }
    */
...
OsdUtilAdaptiveEvaluator::SetCoarsePositions(
        const real *coords, int numFloats, string *errorMessage )
{
...
OsdUtilAdaptiveEvaluator::EvaluateLimit(
    const OsdEvalCoords &coords, real P[3], real dPdu[3], real dPdv[3], real dPdudu[3], real dPdvdv[3], real dPdudv[3])
{
    ...
    cpuEvalLimitController.EvalLimitSample(coords, _evalLimitContext, desc, P, dPdu, dPdv, dPdudu, dPdvdv, dPdudv);
}


void ccgSubSurf__mapGridToFace(int S, real grid_u, real grid_v,
                                      real *face_u, real *face_v)
{
    real u, v;
    ...
    while (not done ) {
        ...
        for (int x = 0; x < currentGridSize; x++) {
            for (int y = 0; y < currentGridSize; y++) {
                real grid_u = (real) x / (currentGridSize - 1),
                      grid_v = (real) y / (currentGridSize - 1);
                ...
                EvaluateLimit(coords, P, NULL, NULL, NULL, NULL, NULL);
