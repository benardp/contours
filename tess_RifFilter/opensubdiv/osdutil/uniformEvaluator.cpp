...
{    
    ...
    if (_refiner->GetTopology().refinementLevel == 0) {
        ...
        memset( _vertexBuffer->BindCpuBuffer(), 0,
                3 * hmesh->GetNumVertices() * sizeof(real));        
        ...
        if (vvNames.size()) {
            ...
            memset( _vvBuffer->BindCpuBuffer(), 0,
                    vvNames.size() * hmesh->GetNumVertices() * sizeof(real));
        }
        return true;
    }
...
OsdUtilUniformEvaluator::SetCoarsePositions(
    const vector<real>& coords, string *errorMessage ) 
{
    const real* pFloats = &coords.front();
...
OsdUtilUniformEvaluator::SetCoarseVVData(
    const vector<real>& data, string *errorMessage
    ) 
{
    if (!_vvBuffer) {
...
OsdUtilUniformEvaluator::GetRefinedPositions(
    const real **positions, int *numFloats,
...
OsdUtilUniformEvaluator::GetRefinedVVData(
    real **data, int *numFloats, int *numElementsRetVal,
