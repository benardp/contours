                             ...
                             std::string *errorMessage)
{
    ...
    if (_t.fvNames.empty()) {
        ...
        // For now we only handle 1 real per FV variable.
...
*/
        ...
        if (!_t.fvNames.empty()) {
            const real* faceData = &(_t.fvData[fvcOffset*fvarWidth]);
...
OsdUtilMesh<T>::GetRefinedFVData(
    int level, const vector<string>& names, vector<real>* outdata)
{
    ...
    for (int i=0; i<(int)faces.size(); ++i) {
        ...
        for (int corner = 0; corner < ncorners; ++corner) {
            ...
            for (int j=0; j<(int)names.size(); ++j) {
                ...
                const real* data = fvariable.GetData(offset);
    ...
    )           
{
    ...
    const real* currentFloat = tagData.floatArgs.size() ? &tagData.floatArgs[0] : NULL;
	...
	switch (tag) {
		...
		if(!e) {
                    ...
                    *errorMessage = ss.str();            
                    ...
                    real sharpness = std::max((real)0.0, ((nfloat > 1) ? currentFloat[j] : currentFloat[0]));
	    ...
	    for(int j = 0; j < nint; ++j) {
                HbrVertex<T>* v = mesh->GetVertex(currentInt[j]);
		if(v) {
            v->SetSharpness(std::max((real)0.0, ((nfloat > 1) ? currentFloat[j] : currentFloat[0])));
		} else {
                    stringstream ss;
