...
typedef struct OpenSubdiv_EvaluatorDescr {
    ...
    // std::vector<real> coarsePositions;
...
int openSubdiv_setEvaluatorCoarsePositions(
    OpenSubdiv_EvaluatorDescr *evaluator_descr,
    const real *positions, int numVertices)
{
    std::string errorMessage;
...
void openSubdiv_evaluateLimit(
    ...
    int face_id, real u, real v,
    real P[3], real dPdu[3], real dPdv[3])
{
    ...
    evaluation_descr->evaluator.EvaluateLimit(coords, P, dPdu, dPdv, NULL, NULL, NULL);
}
