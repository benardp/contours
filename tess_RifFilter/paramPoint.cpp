...
OsdVertexBufferDescriptor g_idesc( /*offset*/ 0, /*length*/ 3, /*stride*/ 3 ),
                          g_odesc( /*offset*/ 0, /*length*/ 3, /*stride*/ 6 );
...
template<class T>
void ParamPoint<T>::Evaluate(OsdEvalCoords coord, vec3 *limitPos, vec3 *tanU, vec3 *tanV, mat2* I, mat2* II)
{
    g_evalCtx->GetVertexData().Bind( g_idesc, g_vertexData, g_odesc, g_Q, g_dQu, g_dQv );

    g_evaluator.EvalLimitSample<OsdCpuVertexBuffer,OsdCpuGLVertexBuffer>(coord,g_evalCtx,1);
...
void ParamPoint<T>::Evaluate(vec3 & limitPosition, vec3 & limitNormal, real * k1, real * k2, vec3 * d1, vec3 * d2) const
{
    ...
    if (IsExactlyEvaluable())
    {
        ...
        if(k1 && k2){
            ...
            ParamPoint<T>::Evaluate(OsdEvalCoords(face->GetID(),u,v),&limitPosition,&tanU,&tanV,&I,&II);
            //_evaluator->Eval(face,u,v,REF_LEVEL,&limitPosition,&tanU,&tanV,&I,&II,NULL,0,NULL,0,NULL,0);
            ...
            ParamPoint<T>::Evaluate(OsdEvalCoords(face->GetID(),u,v),&limitPosition,&tanU,&tanV,NULL,NULL);
