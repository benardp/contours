...
public:
    ...
    // semantics of this are messed up; ought to return Vec(*this).  this method breaks expressions like
    // (a-b).norm().  however, I don't want to change it without checking how it is used. -aaron
    ...
    inline void printFloat() const
    {
        printf("[");
        for(unsigned i =0;i<N-1;i++)
            printf("%f,",(float)this->_coord[i]);
        printf("%f]",(float)this->_coord[N-1]);
    }
...
public:
    ...
    inline void Set(T xform[N][N]) {
        for(int i=0; i<N; i++)
            for(int j=0; j<N; j++)
                (*this)(i,j) = xform[i][j];
    }
    inline void Set(float xform[N][N]) {
        for(int i=0; i<N; i++)
            for(int j=0; j<N; j++)
                (*this)(i,j) = xform[i][j];
    }
...
typedef VecMat::Vec2<real> vec2;
typedef VecMat::Vec2<int> ivec2;
typedef VecMat::Vec3<real> vec3;
typedef VecMat::HVec3<real> vec4;
typedef VecMat::SquareMatrix<real,2> mat2;
typedef VecMat::SquareMatrix<real,4> mat4;

inline real length(vec3 v) { return sqrt(v*v); }
inline real determinant(mat2 m) { return (m[0][0] * m[1][1] - m[0][1] * m[1][0]); }
